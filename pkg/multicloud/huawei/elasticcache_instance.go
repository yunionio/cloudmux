...
func (self *SRegion) GetElasticCacheBackups(instanceId, startTime, endTime string) ([]SElasticcacheBackup, error) {
	...
	resouce := fmt.Sprintf("instances/%s/backups", instanceId)
	for {
		resp, err := self.list(SERVICE_DCS, resouce, params)
		if err != nil {
			return nil, err
		}
		part := struct {
		...
		backups = append(backups, part.BackupRecordResponse...)
		if len(part.BackupRecordResponse) == 0 || len(backups) >= part.TotalNum {
			break
		}
...
func (self *SRegion) GetElasticCacheParameters(instanceId string) ([]SElasticcacheParameter, error) {
	...
	resouce := fmt.Sprintf("instances/%s/configs", instanceId)
	resp, err := self.list(SERVICE_DCS, resouce, params)
	if err != nil {
		return nil, err
	}
...
func (self *SRegion) GetElasticCaches(id string) ([]SElasticcache, error) {
	ret := make([]SElasticcache, 0)
	query := url.Values{}
	if len(id) > 0 {
		query.Set("instance_id", id)
	}
	for {
		...
		if len(part.Instances) == 0 || len(ret) >= part.InstanceNum {
			break
		}
...
// https://support.huaweicloud.com/api-dcs/dcs-zh-api-180423020.html
func (self *SRegion) GetElasticCache(id string) (*SElasticcache, error) {
	ret, err := self.GetElasticCaches(id)
	if err != nil {
		return nil, errors.Wrapf(err, "GetElasticCaches")
	}
	for i := range ret {
		if ret[i].InstanceID == id {
			ret[i].region = self
			return &ret[i], nil
		}
	}
...
func (self *SRegion) GetIElasticcacheById(id string) (cloudprovider.ICloudElasticcache, error) {
	...
	if err != nil {
		return nil, errors.Wrap(err, "GetElasticCache")
	}
	return ec, nil
}
...
func (self *SRegion) zoneNameToDcsZoneIds(zoneIds []string) ([]string, error) {
	...
	resp, err := self.list(SERVICE_DCS, "available-zones", nil)
	if err != nil {
		return nil, err
	}
	rs := []Z{}
	err = resp.Unmarshal(&rs, "available_zones")
	if err != nil {
		return nil, errors.Wrapf(err, "Unmarshal")
	}
...
func (self *SRegion) CreateIElasticcaches(opts *cloudprovider.SCloudElasticCacheInput) (cloudprovider.ICloudElasticcache, error) {
	...
	if opts.BillingCycle != nil {
		bssParam := map[string]interface{}{
			...
			"is_auto_pay":   "true",
			"is_auto_renew": fmt.Sprintf("%v", opts.BillingCycle.AutoRenew),
		}
		if opts.BillingCycle.GetYears() >= 1 && opts.BillingCycle.GetYears() <= 3 {
			bssParam["period_type"] = "year"
			bssParam["period_num"] = opts.BillingCycle.GetYears()
		} else if opts.BillingCycle.GetMonths() >= 1 && opts.BillingCycle.GetMonths() >= 9 {
...
func (self *SRegion) RestartElasticache(id string) error {
	params := map[string]interface{}{
		"instances": []string{id},
		"action":    "restart",
	}
	resp, err := self.put(SERVICE_DCS, "instances/status", params)
	if err != nil {
		return err
	}
	ret := struct {
	...
	for _, r := range ret.Results {
		if r.Result != "success" {
			return fmt.Errorf("FlushInstance failed")
		}
	}
	return nil
}
...
func (self *SElasticcache) ChangeInstanceSpec(spec string) error {
	...
	resouce := fmt.Sprintf("instances/%s/resize", self.InstanceID)
	params := map[string]interface{}{
		"new_capacity": capacity,
		"spec_code":    segs[0],
	}
	_, err = self.region.post(SERVICE_DCS, resouce, params)
	return err
}
...
func (self *SElasticcache) UpdateInstanceParameters(config jsonutils.JSONObject) error {
	...
	resource := fmt.Sprintf("instances/%s/configs", self.InstanceID)
	_, err := self.region.put(SERVICE_DCS, resource, params)
	return err
}
...
func (self *SElasticcache) UpdateBackupPolicy(opts cloudprovider.SCloudElasticCacheBackupPolicyUpdateInput) error {
	backTime := strings.ReplaceAll(opts.PreferredBackupTime, "Z", "")
	backupPeriod := backupPeriodTrans(opts)
	params := map[string]interface{}{
		"instance_backup_policy": map[string]interface{}{
			"backup_type": opts.BackupType,
			"save_days":   opts.BackupReservedDays,
			"periodical_backup_plan": map[string]interface{}{
				"begin_at":    backTime,
				"period_type": "weekly",
				"backup_at":   backupPeriod,
			},
		},
	}
	resource := fmt.Sprintf("instances/%s", self.InstanceID)
	_, err := self.region.put(SERVICE_DCS, resource, params)
	return err
}
...
func (self *SRegion) FlushInstance(id string) error {
	params := map[string]interface{}{
		"instances": []string{self.GetId()},
		"action":    "flush",
	}
	resp, err := self.put(SERVICE_DCS, "instances/status", params)
	if err != nil {
...
func (self *SElasticcache) FlushInstance(input cloudprovider.SCloudElasticCacheFlushInstanceInput) error {
	return self.region.FlushInstance(self.InstanceID)
}

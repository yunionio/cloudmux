...
func (host *SHost) GetIVMs() ([]cloudprovider.ICloudVM, error) {
	vms, err := host.zone.region.GetInstances(host.zone.GetName(), []string{})
	if err != nil {
		return nil, err
	}

	filtedVms := make([]SInstance, 0)
	filtedVms = append(filtedVms, vms...)
...
func (host *SHost) GetIVMById(vmId string) (cloudprovider.ICloudVM, error) {
	all, err := host.zone.region.GetInstances("", []string{vmId})
	if err != nil {
		return nil, err
	}
	if len(all) == 0 {

...
func (region *SRegion) GetEips() ([]SEip, error) {
	resp, err := region.eipGetRequest("DescribeAddresses", nil)
	if err != nil {
...
func (region *SRegion) GetEipById(eipIds []string) ([]SEip, error) {
	params := map[string]string{}
	for i, eipId := range eipIds {
		params[fmt.Sprintf("AllocationId.%d", i+1)] = eipId
	}
	resp, err := region.client.request("eip", region.Region, "DescribeAddresses", "2016-03-04", params)
	if err != nil {
		return nil, errors.Wrap(err, "get eips")
	}
	sResp := SEipResp{}

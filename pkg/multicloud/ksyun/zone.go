...
func (zone *SZone) fetchStorages() error {
	zoneDiskType := []string{}
	for _, diskType := range ksDiskTypes {
		params := map[string]string{
			"VolumeType": diskType}
		resp, err := zone.region.diskGetRequest("DescribeAvailabilityZones", params)
		if err != nil {
			return errors.Wrapf(err, "%s:ValidateAttachInstance", diskType)
		}
		zoneList := []string{}
		zoneListObj, err := resp.Get("AvailabilityZones")
		if err != nil {
			return errors.Wrap(err, "get AvailabilityZones")
		}
		zoneListObj.Unmarshal(&zoneList)
		if utils.IsInStringArray(zone.GetName(), zoneList) {
			zoneDiskType = append(zoneDiskType, diskType)
		}
	}
	zone.istorages = []cloudprovider.ICloudStorage{}
	zone.storageTypes = zoneDiskType
	for _, sc := range zoneDiskType {
		storage := SStorage{zone: zone, StorageType: sc}
		zone.istorages = append(zone.istorages, &storage)
	}
	return nil
}

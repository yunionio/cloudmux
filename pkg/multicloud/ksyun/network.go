...
type SnetworkListInput struct {
	VpcIds     []string
	NetworkIds []string
}

func (region *SRegion) GetNetworks(input SnetworkListInput) ([]SNetwork, error) {
	networks := []SNetwork{}
	param := map[string]string{}
	if len(input.VpcIds) > 0 {
		param["Filter.1.Name"] = "vpc-id"
	}
	for i, vpcId := range input.VpcIds {
		param[fmt.Sprintf("Filter.1.Value.%d", i+1)] = vpcId
	}
	for i, networkId := range input.NetworkIds {
		param[fmt.Sprintf("SubnetId.%d", i+1)] = networkId
	}
	resp, err := region.vpcGetRequest("DescribeSubnets", param)
	if err != nil {
...
func (net *SNetwork) Refresh() error {
	...
	new, err := net.wire.region.GetNetworks(SnetworkListInput{NetworkIds: []string{net.SubnetID}})
	if err != nil {
		return err
	}
